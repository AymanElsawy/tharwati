...
ClassifiedAppError classifyAppError(Object error) {
  ...
  if (error is TimeoutException) {
    return ClassifiedAppError(AppErrorCode.timeout, error);
  }
  if (error is SocketException) {
    return ClassifiedAppError(AppErrorCode.offline, error);
  }
  if (error is AuthException) {
    if (error.statusCode == '403') {
      return ClassifiedAppError(AppErrorCode.forbidden, error);
    }
...
String safeAppErrorMessage(Object error, AppLanguage language) {
  ...
  if (language == AppLanguage.en &&
      error is AppErrorCarrier &&
      error.trustedBusinessMessage != null) {
    return error.trustedBusinessMessage!;
  }
  return safeAppErrorCodeMessage(classified.code, language);
}

String safeAppErrorCodeMessage(AppErrorCode code, AppLanguage language) {
  ...
  return switch (code) {
    ...
    AppErrorCode.businessRule =>
      ar
          ? 'لا يمكن إجراء هذا التغيير في الحالة الحالية.'
    ...
    AppErrorCode.forbidden =>
      ar
          ? 'ليس لديك صلاحية لإجراء ذلك.'
          : 'You do not have permission to do that.',
    AppErrorCode.offline =>
      ar
          ? 'يبدو أنك غير متصل بالإنترنت. تحقق من اتصالك.'
    ...
    AppErrorCode.serviceUnavailable =>
      ar
          ? 'الخدمة غير متاحة الآن. حاول لاحقًا.'
          : 'The service is unavailable right now. Please try again later.',
    AppErrorCode.marketPriceUnavailable =>
      ar
          ? 'سعر السوق غير متاح. لا يمكن حساب القيمة.'
          : 'A market price is unavailable. The value cannot be calculated.',
    AppErrorCode.fxUnavailable =>
      ar
          ? 'سعر الصرف غير متاح. لا يمكن تحويل القيمة.'
          : 'An exchange rate is unavailable. The value cannot be converted.',
    AppErrorCode.fxStale =>
      ar
          ? 'سعر الصرف قديم. راجعه قبل الاعتماد على القيمة.'
          : 'The exchange rate is out of date. Review it before relying on this value.',
