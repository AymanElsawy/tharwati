...
export type AddBrokeragePartialDividendReinvestmentInput = Database["public"]["Functions"]["add_brokerage_partial_dividend_reinvestment"]["Args"]
export async function addBrokeragePartialDividendReinvestment(input: AddBrokeragePartialDividendReinvestmentInput) { await requireAuthenticatedUserId(supabase, "brokerageDividends.partialReinvest"); const { data,error }=await supabase.rpc("add_brokerage_partial_dividend_reinvestment",input); return requireQueryData(data,error,"brokerageDividends.partialReinvest") }
