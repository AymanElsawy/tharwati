...
export function SettingsPage() {
  ...
  return <section className="tharwati-page-stack max-w-3xl">
    ...
    <article className="tharwati-card border border-red-200 p-5 sm:p-6"><div className="flex gap-3"><ShieldAlert className="mt-0.5 size-5 shrink-0 text-red-700" /><div><h2 className="text-lg font-bold text-red-800 dark:text-red-300">{t("settings.delete.title")}</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{t("settings.delete.description")}</p></div></div><DeleteAccountDialog email={user.email} onDownloadData={() => void downloadData()} downloadDisabled={exportStatus === "loading"} /></article>
  </section>
}
