...
import { getValuationMethodLabel } from "@/features/accounts/types/valuation-method"
import {
  attributableValuation,
  getEffectiveAccountValuations,
} from "@/features/accounts/services/account-valuations.service"
import {
  getAccountCurrentOwnership,
  getAccountDisposals,
} from "@/features/accounts/services/account-disposals.service"
import type {
  AccountDisposal,
  AccountOwnershipProjection,
} from "@/features/accounts/types/account-disposal"
...
export function AccountDetailsPage() {
  ...
  const accountValue = account
    ? (accountValues.values.get(account.id) ?? null)
    : null
  ...
  if (
    ...
    return (
      <AccountRecordsPage
        ...
        isAccountValueLoading={accountValues.isLoading}
      />
    )
  if (account.account_type_code === "gold")
    return (
      <MetalAccountDetailsPage
        accountId={account.id}
        accountValue={accountValue}
        isAccountValueLoading={accountValues.isLoading}
        onAccountValueRefresh={accountValues.refresh}
      />
    )
  if (account.account_type_code === "brokerage")
    return (
      <BrokerageAccountDetailsPage
        account={account}
        brokerageValue={accountValues.brokerageValues.get(account.id) ?? null}
        isBrokerageValueLoading={accountValues.isLoading}
      />
    )
  ...
  if (
    ...
    return <ValuedAccountDetailsPage account={account} />
  return (
    <div className="pb-12">
      <Button
        variant="ghost"
      ...
      <AccountValue
        ...
        locale={locale}
        isLoading={accountValues.isLoading}
      />
    </div>
  )
}
...
function ValuedAccountDetailsPage({ account }: { account: AccountSummary }) {
  const navigate = useNavigate()
  const { t, language } = useTranslation()
  const valuationAccountType =
    account.account_type_code === "business" ? "business" : "real_estate"
  const getBusinessValuationMethodLabel = (
    storedMethod: string | null,
    translate: typeof t
  ) => getValuationMethodLabel(valuationAccountType, storedMethod, translate)
  const [valuations, setValuations] = useState<
    Awaited<ReturnType<typeof getEffectiveAccountValuations>>
  >([])
  const [disposals, setDisposals] = useState<AccountDisposal[]>([])
  const [ownership, setOwnership] = useState<AccountOwnershipProjection | null>(
    null
  )
  ...
  const load = useCallback(async () => {
    setIsLoading(true)
    try {
      const [nextValuations, nextOwnership, nextDisposals] = await Promise.all([
        getEffectiveAccountValuations([account.id]),
        getAccountCurrentOwnership([account.id]),
        getAccountDisposals([account.id]),
      ])
      setValuations(nextValuations)
      setOwnership(nextOwnership[0] ?? null)
  ...
  const value = attributableValuation(
    latest,
  ...
  const metadata =
    account.account_type_code === "business"
      ? [
          {
            label: t("accounts.form.businessType"),
            value: getBusinessTypeLabel(account.business_type, t),
          },
          {
            label: t("accounts.form.industry"),
            value: getIndustryLabel(account.industry, t),
          },
          {
            label: t("accounts.form.ownershipPercentage"),
            value: currentOwnership,
          },
          ...(account.notes
            ? [{ label: t("accounts.form.notes"), value: account.notes }]
            : []),
        ]
      : [
          {
            label: t("accounts.form.propertyType.label"),
            value: getPropertyTypeLabel(account.property_type, t),
          },
          {
            label: t("accounts.form.ownershipPercentage"),
            value: currentOwnership,
          ...
          ...(account.location
            ? [{ label: t("accounts.form.location"), value: account.location }]
            : []),
          ...(account.notes
            ? [{ label: t("accounts.form.notes"), value: account.notes }]
            : []),
  ...
  return (
    <div className="pb-12">
      ...
      {refreshStale ? (
        <div
          role="status"
          className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200"
        >
          <span>{t("accounts.records.savedRefreshFailed")}</span>
          <Button
            size="sm"
            variant="outline"
            onClick={() => void load().catch(() => setRefreshStale(true))}
          >
      ...
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-[var(--color-border)] pb-7">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <h1 className="tharwati-page-title">{account.name}</h1>
            {isSold ? (
          ...
          {isSold ? (
            <p className="mt-2 text-sm text-muted-foreground">
              ...
              {account.closed_on ?? sale?.disposedOn ?? "—"}
            </p>
          ) : null}
          <p className="mt-3 text-sm text-muted-foreground">
            {t("accounts.valuation.attributableValue")}
          </p>
          <AccountValue
            value={value}
            currencyCode={account.currency_code}
        ...
        {!isSold ? (
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={() => setIsDisposalOpen(true)}>
              <Tag size={16} />
              {t(
                account.account_type_code === "real_estate"
      ...
      <section className="mt-7 rounded-2xl border border-[var(--color-border)] p-5">
        <h2 className="font-heading text-lg font-semibold">
          {t("accounts.valuation.history")}
        </h2>
        {valuations.length === 0 && !isLoading ? (
          ...
          <div className="mt-4 space-y-3">
            {valuations.map((valuation) => (
              <div
                key={valuation.id}
                className="rounded-xl bg-[var(--color-surface-muted)] p-3"
              >
                <div className="flex justify-between gap-3">
                  <span>{valuation.valuedOn}</span>
                  <span dir="ltr">
                    {t("accounts.valuation.fullValue")}:{" "}
                    {valuation.valuationAmount} {account.currency_code}
                  </span>
                </div>
        ...
        <section className="mt-5 rounded-2xl border border-[var(--color-border)] p-5">
          ...
          <div className="mt-4 space-y-3">
            {disposals.map((disposal) => (
              ...
              >
                <div className="flex justify-between gap-3">
                  <span>{disposal.disposedOn}</span>
                  <span dir="ltr">
                    {disposal.saleAmount} {disposal.saleCurrencyCode}
                  </span>
                </div>
                <p className="mt-1 text-sm text-muted-foreground">
                  {disposal.ownershipPercentageSold}%
                </p>
                {disposal.notes ? (
                  <p className="mt-1 text-sm text-muted-foreground">
                    {disposal.notes}
      ...
      <AccountDisposalDialog
        ...
        currentOwnership={ownership?.ownershipPercentage ?? null}
...
function MetalAccountDetailsPage({
  accountId,
  accountValue,
  isAccountValueLoading,
  onAccountValueRefresh,
}: {
  accountId: string
  accountValue: Decimal | null
  isAccountValueLoading: boolean
  onAccountValueRefresh: () => Promise<void>
}) {
  const navigate = useNavigate()
  const { t, language } = useTranslation()
  const accounts = useAccounts()
  ...
  const valuedPurchases = useMemo(
    () => valueMetalPurchases(purchases, price),
    [purchases, price]
  )
  const purities = useMemo(
    () => aggregateValuedMetalPurchasesByPurity(valuedPurchases),
    [valuedPurchases]
  )
  const locale = language === "ar" ? "ar-SA" : "en-US"
    ...
    <div className="pb-12">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="min-w-0">
            ...
            <AccountValue
              ...
              currencyCode={account.currency_code}
              locale={locale}
              isLoading={isAccountValueLoading}
            />
          </div>
      ...
      <section className="mt-8 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm sm:p-6">
        <MetalPurchaseHistoryContent
          ...
          onOpenPurity={(purity) =>
            navigate(
              `/accounts/${account.id}/purities/${encodeURIComponent(purity)}`
            )
          }
        />
      </section>
