...
export function AssetsBreakdownCard({
  aggregate,
  isLoading,
  embedded = false,
}: {
  ...
  if (isLoading)
    return embedded ? (
      <section
        aria-label={t("dashboard.assetsBreakdown.loading")}
        className="min-w-0"
      >
        <div className="min-h-64 animate-pulse">
          <div className="h-4 w-36 rounded bg-[var(--color-surface-hover)]" />
  ...
  if (!aggregate || aggregate.status === "incomplete")
    return embedded ? (
      <section className="min-w-0">
        <div className="flex items-center gap-2 text-amber-800 dark:text-amber-300">
          <AlertTriangle className="size-5" />
          <h2 className="font-bold">{t("dashboard.assetsBreakdown.title")}</h2>
        </div>
        <p className="mt-3 text-sm text-[var(--color-text-secondary)]">
          {t("dashboard.assetsBreakdown.unavailable")}
        </p>
      </section>
    ) : (
  ...
  const content = (
    <>
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
            {t("dashboard.assetsBreakdown.description")}
          </p>
        </div>
      ...
      {items.length === 0 ? (
        <p className="mt-4 text-sm text-[var(--color-text-secondary)] md:mt-6">
          {t("dashboard.assetsBreakdown.empty")}
        </p>
      ) : (
        <div className="mt-4 grid gap-4 md:mt-5 md:grid-cols-[12rem_minmax(0,1fr)] md:items-center md:gap-5">
          <div
            className="relative mx-auto h-44 min-h-44 w-full max-w-44 min-w-44 md:h-48 md:w-48"
            role="img"
            aria-label={t("dashboard.assetsBreakdown.chartLabel")}
          >
            ...
            <div className="pointer-events-none absolute inset-10 flex flex-col items-center justify-center text-center">
              <span className="text-[10px] font-medium text-[var(--color-text-secondary)]">
                {t("dashboard.assetsBreakdown.totalAssets")}
              </span>
          ...
          <ul
            ...
            aria-label={t("dashboard.assetsBreakdown.legendLabel")}
          >
            {items.map((item) => (
      ...
      <div className="mt-4 flex items-center justify-between border-t border-[var(--color-border)] pt-3 text-sm md:mt-6 md:pt-4">
        <span className="font-medium text-[var(--color-text-secondary)]">
          {t("dashboard.assetsBreakdown.totalLiabilities")}
        </span>
  ...
  return embedded ? (
    ...
    <article className="tharwati-card h-full p-5 sm:p-6">{content}</article>
  )
}
