-- Static/reference catalogue only. This file intentionally contains no rows
-- owned by an auth user and no operational market or exchange-rate data.

insert into public.account_types (code, name, is_active)
values
  ('cash', 'Cash', true),
  ('bank', 'Bank', true),
...
insert into public.asset_types (code, name, is_active)
values
  ('stock', 'Stock', true),
  ('etf', 'ETF', true),
  ...
  ('real_estate', 'Real Estate', true),
  ('business', 'Business', true),
  ('cash_equivalent', 'Cash Equivalent', true),
  ('other', 'Other', true)
on conflict (code) do update
set name = excluded.name, is_active = excluded.is_active;
...
values
  ...
  ('investment_purchase', 'Investment purchase', true),
  ('investment_purchase_reversal', 'Investment purchase reversal', true),
  ...
  ('account_disposal_proceeds', 'Account disposal proceeds', true),
  ('refund', 'Refund', true),
...
values
  ...
  ('EGP', 'Egyptian Pound', 'ج.م', true),
  ('EUR', 'Euro', '€', true),
  ('GBP', 'British Pound', '£', true),
  ('AED', 'United Arab Emirates Dirham', 'د.إ', true)
on conflict (code) do update
set name = excluded.name, symbol = excluded.symbol, is_active = excluded.is_active;

-- System categories only. Custom user categories are deliberately excluded.
