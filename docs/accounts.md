...
Brokerage Account Details supports Stage 1 **Cash Dividend** posting for a positive holding only when its asset currency matches the Brokerage account currency. The user supplies gross dividend, optional withholding tax and fees (both default zero), date/time, and notes. The atomic `add_brokerage_cash_dividend` RPC posts Net Dividend (`gross - tax - fees`) only when it is positive; it credits the same Brokerage Available Cash while recording zero quantity and zero cost-basis effects, so holdings and unrealized P/L are unchanged. Activity shows Dividend, asset, gross, tax, fees, net received, and date. DRIP, reinvestment, and cross-currency dividends are not supported in this stage.
