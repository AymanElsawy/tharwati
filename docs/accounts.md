...
- **Create mode**: two-step — (1) a type picker (radio-group grid of 7 type cards with icon+label), (2) the form itself, pre-seeded with the chosen type. Below `sm`, the picker uses compact 44 px minimum-height rows with existing type icons/colors. At `sm` and above, it retains the two-column card layout. At every breakpoint, each option has a logical-end radio indicator and a subtle green selected state. Only the picker list scrolls when space is short; the dialog title/description/close control and sticky Continue/Cancel footer remain visible.
