...
Brokerage Holding Details and the Brokerage holdings list also show holding-level Unrealized P/L and Unrealized P/L % from the same shared valuation snapshot. Unrealized P/L is current market value minus current cost basis, and the percentage is P/L divided by current cost basis times 100; both use the account-currency valuation basis and remain unavailable when current price or required FX is unavailable. No Brokerage total P/L or realized P/L is shown.
