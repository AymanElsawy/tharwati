...
- All seven application Edge Functions (`dashboard-valuation`, `fx-rates`, `market-prices`, `asset-search`, `delete-account`, `investment-fx`, and `export-my-data`) read the default public key from the Edge runtime's `SUPABASE_PUBLISHABLE_KEYS` JSON dictionary for user-scoped clients, forwarding the caller's `Authorization` JWT where required so RLS remains user-scoped. `fx-rates`, `market-prices`, `investment-fx`, and `delete-account` use the default key from the runtime-only `SUPABASE_SECRET_KEYS` dictionary only for their shared-cache or Auth Admin operations. Missing dictionaries/default entries fail closed without legacy-key fallback. The Web client requires `VITE_SUPABASE_PUBLISHABLE_KEY`; Mobile requires the `SUPABASE_PUBLISHABLE_KEY` Dart define. No client receives the secret key.
...
The live Dashboard aggregate consumes an authenticated `dashboard-valuation` Edge Function snapshot. Its `dashboard_valuation_snapshots` table stores one unexpired snapshot per user and supported base currency (`USD`, `SAR`, `EGP`, `EUR`, `GBP`, or `AED`), and `store_dashboard_valuation_snapshot(...)` atomically preserves the first valid snapshot in the 15-minute window. The final base-currency grouping and liability arithmetic remains in the shared decimal-safe TypeScript aggregate.
