...
4. Success — big total (`{amount} {baseCurrencyCode}`, forced LTR), account count line, an amber warning line if any currency pairs were unavailable, a muted note listing any account types excluded from the total, and a link to the full Accounts tab. Only the resolved main total uses a presentation-only `requestAnimationFrame` count-up: it eases from zero on first display and from the last displayed value on a mounted-value change over 500ms. Loading, error, and unresolved states never animate; reduced-motion users receive the final formatted value immediately. Formatting, currency suffix, and all Net Worth data/calculation behavior are unchanged.
