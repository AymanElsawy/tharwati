...
export function RecordCategoryPicker({ value, onChange, error }: { value: Selection; onChange: (value: Selection) => void; error?: string }) {
  ...
  return <section>
    <div className="flex items-center justify-between gap-3"><label className="text-sm font-semibold">{t("accounts.records.category")}</label><Button type="button" variant="ghost" size="sm" className="h-7 gap-1 px-1.5 text-xs font-medium text-muted-foreground hover:text-[var(--color-text)]" onClick={() => setIsManaging(true)}><Settings size={14}/>{t("accounts.categories.manage")}</Button></div>
...
function SubcategoryOption({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-start text-sm transition-colors ${selected ? "bg-[var(--color-primary)]/10 text-[var(--color-primary)]" : "hover:bg-[var(--color-surface-muted)]"}`}><span className="min-w-0 flex-1 truncate">{label}</span>{selected && <Check size={16} className="shrink-0" />}</button>
}
