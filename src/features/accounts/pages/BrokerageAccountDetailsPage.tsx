...
function activityAssetEntry(activity: BrokerageActivityItem) {
  const partialReinvestmentEntry = activity.entries.find((entry) =>
    entry.memo === "brokerage_dividend_partial_reinvestment"
  )
  if (partialReinvestmentEntry) return partialReinvestmentEntry
...
function activityLabel(item: BrokerageActivityItem, accountId: string, t: (key: TranslationKey) => string) {
  ...
  if (item.transaction_type_code === "dividend") return item.entries.some((entry) => entry.memo === "brokerage_dividend_partial_reinvestment") ? t("brokerage.dividendPartiallyReinvested") : item.entries.some((entry) => entry.memo === "brokerage_dividend_reinvestment") ? t("brokerage.dividendReinvested") : t("brokerage.dividend")
...
function BrokerageActivityDialog({
  ...
  const isPartialReinvestment = !!activity?.entries.some((entry) => entry.memo === "brokerage_dividend_partial_reinvestment")
  const partialReinvestedAmount = activity && isPartialReinvestment
    ? sumEntries(activity.entries, "brokerage_dividend_partial_reinvestment", "account_amount")
    : null
  const partialCashRemainder = activity && isPartialReinvestment
    ? sumEntries(activity.entries, "brokerage_dividend_partial_cash", "account_amount")
    : null
  const dividendNet = isPartialReinvestment && partialReinvestedAmount !== null && partialCashRemainder !== null
    ? addDecimals(partialReinvestedAmount, partialCashRemainder)
    : activity
      ? sumEntries(activity.entries, isDrip ? "brokerage_dividend_reinvestment" : "brokerage_dividend_cash", "account_amount")
      : null
      ...
      <Dialog.Popup className="fixed inset-x-3 top-1/2 z-[80] mx-auto max-h-[calc(100vh-2rem)] w-auto max-w-lg -translate-y-1/2 overflow-y-auto rounded-xl bg-[var(--color-surface)] p-5 shadow-xl sm:inset-x-0">
        ...
        {activity && assetEntry ? <div className="mt-5 space-y-4 text-sm">
          ...
          {isDividend ? <>
            ...
            <ActivityDetail label={t(isDrip || isPartialReinvestment ? "brokerage.netDividendReinvested" : "brokerage.netDividend")} value={formatAmount(dividendNet!, accountCurrency, locale)} />
            ...
            {isPartialReinvestment ? <><ActivityDetail label={t("brokerage.reinvestedAmount")} value={formatAmount(partialReinvestedAmount!, accountCurrency, locale)} /><ActivityDetail label={t("brokerage.cashRemainder")} value={formatAmount(partialCashRemainder!, accountCurrency, locale)} /><ActivityDetail label={t("brokerage.reinvestmentUnitPrice")} value={assetEntry.unit_price === null ? "--" : formatAmount(assetEntry.unit_price, accountCurrency, locale)} /><ActivityDetail label={t("brokerage.quantityAdded")} value={assetEntry.quantity_delta ?? "--"} /></> : null}
          </> : <>
