...
export function DashboardPortfolioAllocationCard({
  ...
  return <article className="tharwati-card h-full p-5 sm:p-6">
    ...
    {items.length === 0 ? <p className="mt-6 text-sm text-[var(--color-text-secondary)]">{t("dashboard.portfolioAllocation.empty")}</p> : <div className="mt-5 grid gap-5 md:grid-cols-[12rem_minmax(0,1fr)] md:items-center"><div className="relative mx-auto h-52 w-full min-h-52 min-w-52 max-w-52 md:h-48 md:w-48" role="img" aria-label={t("dashboard.portfolioAllocation.chartLabel")}><ResponsiveContainer width="100%" height="100%"><RechartsPieChart><Pie data={chartData} dataKey="chartValue" nameKey="group" innerRadius={58} outerRadius={84} paddingAngle={2} strokeWidth={0}>{chartData.map((item) => <Cell key={item.group} fill={item.color} />)}</Pie></RechartsPieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-10 flex flex-col items-center justify-center text-center"><span className="text-[10px] font-medium text-[var(--color-text-secondary)]">{t("dashboard.portfolioAllocation.totalInvestments")}</span><strong className="mt-1 text-sm font-bold tabular-nums" dir="ltr">{formatPortfolioAmount(totalInvestments, baseCurrencyCode, locale)}</strong></div></div><ul className="grid gap-3" aria-label={t("dashboard.portfolioAllocation.legendLabel")}>{items.map((item) => <li key={item.group} className="min-w-0"><div className="flex min-w-0 items-start justify-between gap-3 md:hidden"><div className="flex min-w-0 items-center gap-2"><span aria-hidden="true" className="size-2.5 shrink-0 rounded-full" style={{ backgroundColor: item.color }} /><span className="min-w-0 break-words text-sm">{t(item.labelKey)}</span></div><div className="min-w-0 max-w-[58%] shrink-0 text-end"><span className="block break-words text-sm tabular-nums" dir="ltr">{formatPortfolioAmount(item.value, baseCurrencyCode, locale)}</span><strong className="mt-0.5 block text-xs font-bold tabular-nums text-[var(--color-text-primary)]" dir="ltr">{formatPortfolioPercent(item.percentage, locale)}</strong></div></div><div className="hidden min-w-0 grid-cols-[auto_minmax(0,1fr)_auto_auto] items-center gap-x-2 md:grid"><span aria-hidden="true" className="size-2.5 shrink-0 rounded-full" style={{ backgroundColor: item.color }} /><span className="min-w-0 break-words text-sm">{t(item.labelKey)}</span><span className="shrink-0 text-xs font-semibold tabular-nums" dir="ltr">{formatPortfolioPercent(item.percentage, locale)}</span><span className="min-w-0 break-words text-sm tabular-nums" dir="ltr">{formatPortfolioAmount(item.value, baseCurrencyCode, locale)}</span></div></li>)}</ul></div>}
  </article>
}
